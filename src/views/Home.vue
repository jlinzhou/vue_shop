        ...
        <el-container>
            ...
            <el-aside :width="isCollapse ? '64px':'200px'">
                ...
                <el-menu background-color="#373d41" text-color="#fff" active-text-color="#409eff" :unique-opened="true" :default-active="activePath" :collapse="isCollapse" :collapse-transition="false" router>
                    <el-submenu :index="item.meta.name+''" v-for="item in menulist" :key="item.meta.name">
                        <template slot="title">
                            <!-- <i :class="iconsObj[item.id]"></i> -->
                            <span>{{item.meta.title}}</span>
                        </template>
                        <!-- <span>{{item.children}}</span> -->
                        <el-menu-item :index="'/'+subItem.path" v-for="subItem in item.children" :key="subItem.meta.name" @click="saveNavState('/'+subItem.path)">
                            <template slot="title" v-for="itema in subItem.children" :key="itema.meta.name">
                                <!-- <template v-if="!itema.hidden"> -->
                                <!-- <i class="el-icon-menu"></i> -->
                                <span v-if="!itema.hidden">{{itema.meta.title}}</span>
                                <!-- </template> -->
                            </template>
                        </el-menu-item>
